...
**Медленный, а значит безопасный, сбор энергии ядерного распада и синтеза с прямым переводом в электроэнергию.**

- «Медленный» значит: никакой цепной реакции, разгона, взрыва или высоких температур. Мощность ограничена самой физикой процесса.
- Основной механизм — **сбор заряженных продуктов ядерных реакций электрическим полем** (этапы 4–5), без промежуточного тепла и паровой турбины.
- Полупроводники (алмаз) — запасная ветка для маломощных β-источников (H-3, Ni-63, C-14).
...
## Этап 7. Энергобаланс «медленного» синтеза: `slow_fusion.py`

![slow_fusion](slow_fusion.png)

**Вопрос:** можно ли получить синтез с выигрышем энергии, не нагревая вещество до сотен миллионов градусов? Коэффициент Q — это энергия синтеза, делённая на затраченную энергию. Сечения реакций взяты из справочных аппроксимаций (Бош–Хейл, NRL, Невинс–Суэйн), торможение — по Андерсену–Циглеру (холодная мишень) и Спитцеру (плазма).

**Порог окупаемости при сборе заряженных продуктов полем** (КПД поля 0,7, ускорителя 0,9): Q > 1/(f_заряж·η_поля·η_уск).

| Реакция | Доля энергии в заряженных частицах | Нужно Q |
|---|---|---|
| D + T | 20% (80% уносит нейтрон) | > 7,9 |
| D + D | 66% | > 2,4 |
| D + He-3 | 95% | > 1,7 |
| p + B-11 | 100% | > 1,6 |
...
| Схема | Лучший Q | Нехватка до порога |
|---|---|---|
| Пучок в холодную мишень, D→T | 0,008 (185 кэВ) | ×1000 |
| Пучок в холодную мишень, D→He3 / p→B11 | 0,001 / 0,0006 | ×1500–3000 |
| Пучок в горячую плазму, D→T (10–100 кэВ) | 2–4 | ×2–4 (нейтроны) |
| Пучок в горячую плазму, D→He3 / p→B11 | 0,6 / 0,4 | ×3–4, при этом плазму нужно держать горячей |
| Мюонный катализ d-t (комнатная температура) | Q_эл ≈ 0,1–0,3 | ×3–10 |

Проверка модели: выход D→T при 150 кэВ получился 6,8·10⁻⁵ на дейтрон, у реальных генераторов нейтронов 10⁻⁵–10⁻⁴.

**Выводы:**
1. **Пучок в холодное вещество не окупится никогда.** Ион тратит энергию на электроны мишени в ~10⁴ раз быстрее, чем успевает слиться с ядром. Это следствие кулоновского барьера, а не техники. Нехватка — три порядка.
2. **В горячей плазме Q растёт**: горячие электроны почти не тормозят ион. Но это уже термоядерный режим (10–100 кэВ, то есть 100 млн – 1 млрд градусов), а не «медленный». Даже там безнейтронные реакции D-He3 и p-B11 не дотягивают до порога, а затраты на поддержание плазмы здесь не учтены.
3. **Мюонный катализ — единственный настоящий «холодный» синтез.** Он идёт при комнатной температуре и без плазмы, но пока даёт в 3–10 раз меньше, чем нужно. Ограничения: мюон «прилипает» к альфа-частице (≈0,45%), живёт 2,2 мкс и стоит ~5 ГэВ. Для окупаемости нужно ~680 синтезов на мюон против ~120 сейчас. Рычаги: уменьшить прилипание, ускорить цикл, удешевить мюоны. Даже при прилипании 0,1% и цикле в 4 раза быстрее выходит Q_эл ≈ 0,5–1,3. Кроме того, 80% энергии d-t уносят нейтроны, которые поле не ловит.
4. **Важная переоценка «безопасности».** Синтез безопасен и без медленности: у него нет цепной реакции, в реакторе в любой момент всего граммы топлива, и при любом сбое плазма гаснет за миллисекунды. «Медленный» для синтеза значит «невыгодный». Для распада «медленный» — естественное свойство.

**Итог для цели исследования:**
- **Распад** — единственный источник, который одновременно медленный, безопасный и положительный. Его энергию поле забирает с КПД 65–80% (этапы 4–5).
- **Синтез** с известной физикой медленным и выгодным одновременно не бывает. Ближе всего мюонный катализ (нехватка ×3–10), и это самое перспективное направление для теоретической работы.

**Ограничения:** торможение в холодных мишенях из гелия и бора масштабировано грубо (±30%), сечение p-B11 выше 400 кэВ — ±50%. В плазменной схеме не учтены излучение и затраты на нагрев (это в пользу синтеза). Параметры мюонного катализа — порядковые (литературные ωs ≈ 0,3–0,9%, цена мюона 3–8 ГэВ).

## Следующие шаги (на выбор)

- **Е.** Мюонный катализ глубже: физика прилипания, реактивации и цикла, какие параметры реально сдвигают X, и сбор α-частиц полем.
- **Ж.** Сбор продуктов синтеза полем (протоны 14,7 МэВ, α-частицы) — модель этапа 4 для D-He3 и p-B11.
- **З.** Сводный отчёт по этапам 1–7.
